...
export default {
  ...
  data () {
    return {
      ...
      columns: [{
        ...
        key: 'level4',
        width: 200,
        scopedSlots: { customRender: 'child4' }
      }],
  ...
  created () {
    this.loading = true
    this.$http.get('/rsRole/getMenuAction')
      .then(res => {
  ...
  methods: {
    ...
    getRole () {
      this.loading = true
      this.$http.get('/rsRole/getRole', { params: { roleId: this.roleId } })
        .then(res => {
          if (res.success && res.data) {
            ...
            res.data.forEach(id => {
              if (this.allChecks[id] !== undefined) {
                this.$set(this.allChecks, id, true)
              }
            })
    ...
    handleSubmit () {
      ...
      this.loading = true
      this.$http.post('/rsRole/setRole', Object.assign({ menuArr: [...menuArr], roleId: this.roleId }))
        .then(res => {
          if (res.success && res.data) {
